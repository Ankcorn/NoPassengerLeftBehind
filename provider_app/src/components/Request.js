...
function Request({ name, comment, phone, start, stop, pic, helping, toc, delayed, end_time, start_time }) {
  return (
    <article className="bg-white block w-auto px-4 py-2 rounded-lg shadow flex w-full my-8">
      <img src={pic} alt="" className="h-24 w-24 rounded-full object-cover border-white border-width-2 self-center" />
      ...
      <div className="container mx-auto flex-1 w-auto flex-no-wrap flex">
        <div className="mx-6 flex justify-center inline-block w-auto items-center">
          ...
          <span className={`rounded-lg bg-${getStatus(helping, 0).color}-600 px-3 py-1 shadow`}>{getStatus(helping, 0).text}</span>
        </div>
        <div className="mx-6 flex justify-center items-center">
          ...
          <span className={`rounded-lg bg-${getStatus(helping, 1).color}-600 px-3 py-1 shadow`}>{getStatus(helping, 1).text}</span>
        </div>
        <div className="mx-6 flex justify-center w-256 items-center">
          <div className="text-gray-900 mx-2 my-2 block font-bold"> <div>{stop}</div> <div> {end_time} </div>{delayed === "stop" && <div className="flex"><WarningIcon className="text-red-600" /> Delayed</div>}</div>
          <span className={`rounded-full bg-${getStatus(helping, 2).color}-600 shadow p-2 inline-block w-auto mx-2`}>
            <AccountBalanceIcon />
          </span>
          <span className={`rounded-lg bg-${getStatus(helping, 2).color}-600 px-3 py-1 shadow`}>{getStatus(helping, 2).text}</span>
        </div>
      </div>
      <div className="container mx-auto flex items-center"><button className="rounded-sm bg-gray-800 text-white p-3">Help at {getHelpAtState(helping, [start, toc, stop])}</button></div>
